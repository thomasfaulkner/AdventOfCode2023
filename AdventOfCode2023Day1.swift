//
//  AdventOfCode2023Day1.swift
//  
//
//  Created by Thomas on 12/3/23.
//
...
func separateByNewLine(string: String) -> [String] {
    // Separate the input (one long string) into an array of strings, separating elements by newline (\n).
    return string.components(separatedBy: "\n")
}

func getFirstNumberCharacter(fromString string: String) -> Character? {
    // Return the first character in a string that is able to be typecast to an Integer; i.e., return the first number character, if there is one.
    // If there isn't one, return nil.
    return string.first { Int("\($0)") != nil }
}

func getLastNumberCharacter(fromString string: String) -> Character? {
    // Get the last character in a string by reversing the string then using the same function used to find the first character.
    let reversedInput = String(string.reversed())
    
    return getFirstNumberCharacter(fromString: reversedInput)
}

func compileFoundNumbers(first: Character?, last: Character?) -> Int? {
    // This function accepts two optional characters. The inputs will be either integer characters that can be successfully typecast to an Int, or they will be nil.
    // Check if both characters are nil; if they aren't nil, append them to the compiled string.
    
    // Try to typecast the compiled string to an Int:
    // - If both characters were integers, return the compiled number as an Int
    // - If only one was an integer, return that number as an Int
    // - If the provided string isn't typecast-able, then both characters were nil
    
    var compiledString = ""
    if let first = first {
        compiledString.append(first)
    }
    
    if let last = last {
        compiledString.append(last)
    }
    
    return Int(compiledString)
}

func getCompiledInteger(fromString string: String) -> Int? {
    // The combined function that performs all of the necessary operations to get a combined, two-digit integer from an input string.
    // If there are no integer characters in the string, it will return nil.
    
    let first = getFirstNumberCharacter(fromString: string)
    let last = getLastNumberCharacter(fromString: string)
    
    return compileFoundNumbers(first: first, last: last)
}

func getSumOfIntegers(inUnseparatedString unseparatedString: String) -> Int {
    // The final output sum to be returned
    var sum = 0
    
    // Separate the input string into an array of strings
    let separatedStrings = separateByNewLine(string: unseparatedString)
    
    // For each string, if the string contains at least one number character, make a combined Integer out of the first and last number character in the string. Add that number to the output sum.
    // If the string does not contain a number character, don't add anything to the sum.
    
    for string in separatedStrings {
        if let compiledInteger = getCompiledInteger(fromString: string) {
            sum += compiledInteger
        }
    }
...
getSumOfIntegers(inUnseparatedString: testString)
